...
static void video_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
    ...
    //test_save_image(userdata->image, 1);
...
int main(int argc, char** argv) {
    ...
    userdata.preview_width = 640;
    userdata.preview_height = 360;
    userdata.video_width = 640;
    userdata.video_height = 360;
    userdata.opencv_width = 640;
    userdata.opencv_height = 360;
    ...
    //test_save_image(userdata.image, 0);
    if (!userdata.cascade) {
...
delay(2000);
    while (1) {
        if (vcos_semaphore_wait(&(userdata.complete_semaphore)) == VCOS_SUCCESS) {
            if(MyProject(userdata.image)<0)return -2;
        }
    }
    return 0;
}
